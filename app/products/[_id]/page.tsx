...
const page = async ({ params }: { params: { _id: string } }) => {
  ...
  return (
    <Container scroll className="border-2 space-y-4 pb-24">
      <Button className="fixed bottom-0 right-4 w-[4rem] h-[4rem] rounded-xl">
      ...
      <Container className="!p-0 grid grid-cols-2 gap-4">
        ...
        <SingleStat className="col-span-2" icon={<Calendar1Icon className="size-6" />} title="Restock" value={`${formatDate(product.restockDate)}`} />
        <SingleStat
