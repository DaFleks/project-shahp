...
export async function GET(reqest: NextRequest): Promise<NextResponse> {
  try {
    //  Search input
    ...
    const products = await Product.find({ name: { $regex: q, $options: "i" } }); // Fetch all products from MongoDB
